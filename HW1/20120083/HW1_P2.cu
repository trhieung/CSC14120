		...
		bool useDevice=false, dim3 blockSize=dim3(1, 1))
{
	...
	else // Use device
	{
		...
		CHECK(cudaMalloc(&d_filter, nBytes_filter));
		CHECK(cudaMalloc(&d_inPixels, nBytes));
		CHECK(cudaMalloc(&d_outPixels, nBytes));

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_filter, filter, nBytes_filter, cudaMemcpyHostToDevice));
