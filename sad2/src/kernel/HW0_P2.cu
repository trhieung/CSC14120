...
void HW0_P2(){
    ...
    printf("Vector Size |    Host Time (ms)    | Device Time (ms) (Version 1) | Device Time (ms) (Version 2)\n");
    for (int i = 0; i < 10; i++){
        list_n_test[i] = pow(2, i*2+6);

        printf("%10d", list_n_test[i]);
        evaluateTime(list_n_test[i]);
    }
}