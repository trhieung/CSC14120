            ...
            bool useDevice = false, dim3 blockSize = dim3(1), int nStreams = 1)
{
    ...
    else // Use device
    {
        ...
        int elementsPerStream = (n + nStreams - 1) / nStreams;
        int offset;
        int streamElements;

        for (int i = 0; i < nStreams; ++i)
        {
            offset = i * elementsPerStream;
            streamElements = min(elementsPerStream, n - offset);

            // Copy data to device
            CHECK(cudaMemcpyAsync(&d_in1[offset], &in1[offset], streamElements * sizeof(int), cudaMemcpyHostToDevice, streams[i]));
            CHECK(cudaMemcpyAsync(&d_in2[offset], &in2[offset], streamElements * sizeof(int), cudaMemcpyHostToDevice, streams[i]));

            // Launch kernel
            addVecKernel<<<(streamElements + blockSize.x - 1) / blockSize.x, blockSize, 0, streams[i]>>>(&d_in1[offset], &d_in2[offset], streamElements, &d_out[offset]);

            // Copy result back to host
            CHECK(cudaMemcpyAsync(&out[offset], &d_out[offset], streamElements * sizeof(int), cudaMemcpyDeviceToHost, streams[i]));
